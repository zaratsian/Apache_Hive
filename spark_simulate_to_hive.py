...
def sim_rate():
    import random
    return random.random() * random.triangular(0.1,20000,0.1)

def sim_rate2():
    import random
    return random.random() * random.triangular(0,50,2)
...
def sim_age():
    import random
    return int(random.triangular(15,80,30))
...
def sim_customer_id():
    import random
    return int(random.triangular(1000000,1999999,1000000))

def sim_status():
    import random
    return ['Silver','Gold','Platinum'][int(random.triangular(0,3,0))]

def sim_bool3():
    import random
    return int(random.choice([1]*2 + [0]*18))

udf_random      = udf(sim_random, FloatType())
udf_rate        = udf(sim_rate, FloatType())
udf_rate2       = udf(sim_rate2, FloatType())
...
udf_customer_id = udf(sim_customer_id, IntegerType())
udf_status      = udf(sim_status, StringType())
udf_number_of_contacts = udf(sim_bool3, IntegerType())

sim = df \
  .withColumn('date', udf_date() ) \
  .withColumn('customer_id', udf_customer_id() ) \
  .withColumn('name', udf_name() ) \
  .withColumn('age', udf_age() ) \
  .withColumn('gender', udf_gender() ) \
  .withColumn('state', udf_state() ) \
  .withColumn('status', udf_status() ) \
  .withColumn('transaction', udf_rate() ) \
  .withColumn('called_agent', udf_number_of_contacts() ) \
